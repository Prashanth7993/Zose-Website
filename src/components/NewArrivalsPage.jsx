...
export default function NewArrivalsPage({ products = [], isLoading = false }) {
  ...
  const enrichedProducts = useMemo(
    () =>
      products.map((item, index) => ({
        ...
        isNewArrival: item.badge === "New Arrival" || item.tag === "New",
      })),
    [products]
  );
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 py-8 sm:py-10 lg:py-12">
      <div className="text-center mb-8">
        <p className="text-[10px] sm:text-[11px] tracking-[0.22em] uppercase text-[#C9A14A] mb-2">Just Landed</p>
        ...
        <div>
          ...
          {!filteredProducts.length && !isLoading && (
            <div className="text-center py-16">
              ...
              <p className="text-[12px] text-[#888888] mt-2">Check back soon for new arrivals</p>
            </div>
          )}
        </div>
      </div>
