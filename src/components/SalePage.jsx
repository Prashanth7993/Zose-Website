...
export default function SalePage({ products = [], isLoading = false }) {
  ...
  const enrichedProducts = useMemo(
    () =>
      products.map((item, index) => ({
        ...
        discountPercent: item.originalPrice > item.offerPrice
          ? Math.round(((item.originalPrice - item.offerPrice) / item.originalPrice) * 100)
          : 0,
        isOnSale: item.originalPrice > item.offerPrice && Math.round(((item.originalPrice - item.offerPrice) / item.originalPrice) * 100) >= 20,
      })),
    [products]
  );
  ...
  }, [enrichedProducts, minPrice, maxPrice, selectedFits, selectedColors, selectedGender, selectedSizes, sortBy]);
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 py-8 sm:py-10 lg:py-12">
      <div className="text-center mb-8">
        <p className="text-[10px] sm:text-[11px] tracking-[0.22em] uppercase text-[#C9A14A] mb-2">Limited Time Offers</p>
        <h1 className="text-3xl sm:text-4xl lg:text-5xl text-[#0A0A0A] mb-3" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
          Sale
        </h1>
        <div className="w-12 h-px mx-auto bg-gradient-to-r from-transparent via-[#C9A14A] to-transparent" />
        <p className="text-[13px] sm:text-[14px] text-[#555555] mt-4">Exclusive discounts on premium t-shirts</p>
      </div>
        ...
        <div>
          ...
          >
            <p className="text-[13px] sm:text-[14px] text-[#333333]">
              {filteredProducts.length} offer{filteredProducts.length === 1 ? "" : "s"} available
            </p>
              ...
              {sortOpen && (
                <div className="absolute right-0 top-full mt-2 w-full min-w-[200px] bg-white border border-[#C9A14A]/30 rounded-2xl shadow-lg z-20 overflow-hidden">
                  {[
                    ...
                    { key: "newest", label: "Best Discount" },
                  ].map((option) => (
                    <button
                      key={option.key}
                      type="button"
          ...
          {!filteredProducts.length && !isLoading && (
            <div className="text-center py-16">
              <p className="text-[15px] text-[#555555]" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                No sale items available at the moment
              </p>
              <p className="text-[12px] text-[#888888] mt-2">Check back soon for exclusive offers</p>
            </div>
          )}
        </div>
      </div>
